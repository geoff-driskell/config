...
class Validator:
    ...
    SCHEMA = Schema({
        ...
        Optional('just_left_delay', default=TIME_5_MINUTES): utils.parse_duration_literal,
        Optional('just_arrived_delay', default=TIME_5_MINUTES): utils.parse_duration_literal,
        Optional('extended_away_delay', default=TIME_24_HOURS): utils.parse_duration_literal
    }, extra=True)
...
class Presence(hass.Hass):
    ...
    def _schedule_state_change(self, delay, new_state):
        self.timer = self.run_in(self._on_scheduled_state_change, delay=delay, new_state=new_state)
    ...
    def _init_current_state(self):
        ...
        tracker_state = TrackerState.from_string(self.get_state(entity_id=self._tracker_entity))
        if tracker_state is TrackerState.Home and current_state not in (State.JustArrived, State.Home):
            self.log(f"Current state '{current_state}' is invalid with tracker state '{tracker_state}'. Resetting to home.")
            current_state = State.Home
        elif tracker_state is TrackerState.NotHome and current_state not in (State.JustLeft, State.Away, State.ExtendedAway):
            self.log(f"Current state '{current_state}' is invalid with tracker state '{tracker_state}'. Resetting to away.")
